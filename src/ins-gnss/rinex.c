...
static double uravalue_rnx(int sva)
{
    return 0<=sva&&sva<15?ura_nominal[sva]:8192.0;
}
/* ura value (m) to ura index ------------------------------------------------*/
static int uraindex_rnx(double value)
{
    int i;
                      ...
                      eph_t *eph)
{
    ...
    *eph=eph0;
    ...
    if (sys==SYS_GPS||sys==SYS_QZS) {
        ...
        eph->sva=uraindex_rnx(data[23]);  /* ura (m->index) */
...
extern int outrnxnavb(FILE *fp, const rnxopt_t *opt, const eph_t *eph)
{
    ...
    outnavf(fp,uravalue_rnx(eph->sva));
    outnavf(fp,eph->svh    );
...
extern int outrnxhnavb(FILE *fp, const rnxopt_t *opt, const seph_t *seph)
{
    ...
    outnavf(fp,uravalue_rnx(seph->sva));
    fprintf(fp,"\n%s",sep         );
