...
#define time2str(t,s,n) (time2str((t),(s),(n)),(s))
#define SQR(x)      ((x)*(x))
...
/* satellite/code to carrier frequency --------------------------------------*/
static double sat2freq(int sat, unsigned char code, const nav_t *nav)
{
...
static int readbiaf(const char *file, nav_t *nav)
{
    FILE *fp;
    double cbias;
    char buff[256],bias[4]="",prn[4]="",obs1[4]="",obs2[4];
    int sat,sys_ix,frq_ix,code1,code2,bias_ix1,bias_ix2,sys,freq;
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if (!(code1=obs2code(&obs1[1],&freq))) continue; /* skip if code not valid */
        ...
        if (strcmp(bias,"OSB")==0) {
            ...
            if (!(code2=obs2code(&obs2[1],&freq))) continue; /* skip if code not valid */
...
* read satellite fractional cycle bias parameters
