...
/* help text -----------------------------------------------------------------*/
static const char *usage[] = {"usage: rtkrcv [-s][-nc][-p port][-d dev][-o file][-w pwd][-r level][-t level][-sta sta]",
                              "options",
...
#define FMTOPT                                                                                                         \
    ...
    "oem6-raw,37:igv-sim-feat,38:igv-sim-imu,39:igv-sim-gnss,40:igv-sim-featall,42:ysasm330"
...
/* print message to console or stderr ----------------------------------------*/
static void printmsg(vt_t *vt, const char *format, ...)
{
    va_list ap;
    char buff[MAXSTR * 4];
...
static void readant(vt_t *vt, prcopt_t *opt, nav_t *nav)
{
    ...
    if (readpcv(filopt.rcvantp, &pcvr))
    {
        for (i = 0; i < 2; i++)
        {
            ...
            {
                printmsg(vt, "no antenna %s in %s", opt->anttype[i], filopt.rcvantp);
                continue;
            }
            opt->pcvr[i] = *pcv;
        }
        ...
        printmsg(vt, "antenna file open error %s", filopt.rcvantp);
    ...
    if (readpcv(filopt.satantp, &pcvs))
    {
        ...
        printmsg(vt, "antenna file open error %s", filopt.satantp);
...
static int startsvr(vt_t *vt)
{
    ...
    if (*startcmd && (ret = system(startcmd)))
    {
        ...
        printmsg(vt, "command exec error: %s (%d)\n", startcmd, ret);
    }
    /* virtual console */
    svr.vt = vt;

    /* start rtk server */
    if (!carvigsvrstart(&svr, svrcycle, buffsize, strtype, paths, strfmt, navmsgsel, cmds, cmds_periodic, ropts,
                        nmeacycle, nmeareq, npos, &prcopt, solopt, &moni, errmsg))
    {

        trace(2, "rtk server start error (%s)\n", errmsg);
        printmsg(vt, "rtk server start error (%s)\n", errmsg);
        return 0;
    }
    return 1;
}
...
static void *con_thread(void *arg)
{
    ...
    vt_printf(con->vt, "\n%s** %s ver.%s %s console (h:help) **%s\n", ESC_BOLD, PRGNAME, "1.0.0", PATCH_LEVEL,
              ESC_RESET);
...
/* carvig server main --------------------------------------------------------
 * sysnopsis
 *     carvignav [-s][-nc][-p port][-d dev][-o file][-r level][-t level][-sta sta]
 *
 * description
...
{
    con_t *con[MAXCON] = {0};
    int i, start = 0, noconsole = 0, port = 0, outstat = 0, trace = 0, sock = 0;
